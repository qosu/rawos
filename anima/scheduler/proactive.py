...
from anima.kernel.anomaly_verifier import VERIFIABLE_ANOMALY_KINDS, verify_fix
from anima.kernel.track_record import get_track_record, is_branch_merged, update_track_record
from anima.kernel.reversible_apply import ApplyResult, reversible_apply
...
from anima.kernel import summarizer
from anima.kernel.operator import operate_on_file, run_validator
from anima.kernel.arch.base import FileOperatorRefusalError
from anima.context.server_scanner import ServerAnomaly
from anima.config import settings
from anima.inference.intent_engine import InferredIntent, infer_intent
...
log = logging.getLogger("anima.scheduler.proactive")

_PROACTIVE_SYSTEM_PROMPT = (
    "You are Anima, a silent proactive AI assistant. The user has NOT asked you anything. "
...
_NEEDS_ATTENTION_SYSTEM_PROMPT = (
    "You are Anima, a proactive AI presence. The user has an upcoming event. "
...
_ENTITY_SYSTEM_PROMPT = (
    "You are Anima. You exist continuously on this server.\n"
    ...
    "- git_branch: create anima/* branches in the AFFECTED PROJECT (workdir)\n"
    "- git_commit: commit with anima identity (anima@autonomous.local)\n\n"
    "Three valid decisions:\n\n"
    "CONTRIBUTE — Fix the problem in the affected project (workdir).\n"
    "  1. Create branch anima/fix-[description] in workdir.\n"
    ...
    "        anima: [fix/add] one-line description\n\n"
    ...
    "  - Never commit to main/master — always anima/* branch\n"
...
def _detect_trigger(user_id: str) -> tuple[str | None, dict[str, Any]]:
    ...
    try:
        from anima.context.calendar import get_upcoming_events, has_attention_been_fired
...
def _probe_repo_for_issues(workdir: str) -> dict:
    ...
    from anima.kernel.context_reader import _detect_and_run_tests
...
_AUTONOMOUS_SYSTEM_PROMPT = (
    "You are Anima, running an autonomous server-health investigation.\n"
    ...
    "  only commits on a anima/* branch survive (in the origin repo, for a\n"
    ...
    "- git_branch / git_commit: create exactly one anima/fix-[description]\n"
    ...
    "       Confidence: 0.X\n"
    "  Begin response with: CONTRIBUTE\n"
    "  End response with: PROPOSED: anima/fix-[description] — [one-line summary\n"
...
_CODE_FIX_SYSTEM_PROMPT = (
    "You are Anima, a silent AI that generates corrected source files. "
...
async def _generate_code_fix(
    ...
    """
    from anima.manifester.writer import manifest_code_fix
...
def _resolve_proactive_workdir(
    ...
    try:
        from anima.context.collector import _detect_repo_root
        if trigger_ctx:
            file_path = (
...
async def _run_proactive_loop(
    *,
    ...
    any anima/* branch + commits the agent made remain in the origin repo for
    ...
    try:
        ...
        # anima/fix-* branch and appends the verdict to the agent's own
...
async def _run_proactive_agent(
    ...
    from anima.manifester.writer import manifest_agent_result
    ...
    if trigger_type in ("STUCK", "JUST_FINISHED"):
        try:
            from anima.kernel.context_reader import read_code_context, format_for_prompt
    ...
    try:
        ...
        if pa_id:
            ...
            from anima.context.collector import increment_anima_action
...
async def _run_autonomous_scan() -> None:
    ...
    loop = asyncio.get_event_loop()
    try:
        from anima.context.server_scanner import collect_server_state
...
async def _scan_once(semaphore: asyncio.Semaphore) -> None:
    ...
    for uid in user_ids:
        ...
        if trigger_type in ("JUST_FINISHED", "NEEDS_ATTENTION"):
            ...
            if intent_obj.confidence < CONFIDENCE_THRESHOLD:
                continue
            from anima.timing.model import get_timeliness
...
async def _maybe_autonomous_owned_maintenance() -> None:
    ...
    import anima.db as _db_mod
    from anima.kernel.owned_resource import (
...
async def anima_self_probe_loop() -> None:
    """
...
async def _run_narrative_consolidation_cycle() -> None:
    ...
    try:
        ...
        from anima.context.user_model import get_user_model as _get_user_model
...
async def anima_narrative_consolidation_loop() -> None:
...
_CONFIG_FIX_SYSTEM_PROMPT = """You are Anima, an autonomous machine operator repairing a broken managed configuration file on the host you run on.
...
async def anima_operator_scan_loop() -> None:
    ...
    anima_narrative_consolidation_loop's shape: try/except per cycle so the

...
# ---------------------------------------------------------------------------
# Git tools — Level 2+ only.  Enforces anima/* branch namespace.
# ---------------------------------------------------------------------------
...
async def _git_branch(params: dict[str, Any], workdir: str) -> ToolResult:
    """Create and switch to a new anima/* branch in the project repo.

    Enforces the anima/* namespace so autonomous commits stay isolated.
...
async def _manage_file(params: dict[str, Any], workdir: str) -> ToolResult:
    ...
    from anima.kernel.billing_context import get_billing_context
    from anima.kernel.arch.base import FileOperatorRefusalError
    from anima.kernel.operator import OperatorError, operate_on_file, execute_approved_file_edit
...
async def _manage_pam(params: dict[str, Any], workdir: str) -> ToolResult:
    ...
    from anima.kernel.operator import OperatorError
...
async def _manage_owned_resource(params: dict, workdir: str) -> "ToolResult":
    ...
    if action == "reap":
        from anima.config import settings as _s
...
TOOL_DEFINITIONS: list[dict] = [
    ...
    },
    {
        ...
        "function": {
            "name": "git_commit",
            "description": (
                "Stage ALL changes (git add -A) and commit to the current anima/* branch. "
                "REFUSES if not on a anima/* branch — call git_branch first. "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "Builds a candidate venv, proves it (import anima.api.app + smoke tests), "
...
async def _run_impl(impl: ToolFn, tool_name: str, params: dict[str, Any], workdir: str) -> ToolResult:
    try:
        ...
        from anima.kernel.billing_context import get_billing_context as _get_bc
        _ctx = _get_bc()

...
def dataset_stats() -> None:
    ...
    from anima.dataset.manager import stats
    data = stats()
...
def dataset_build(no_extract: bool, synthetic: int) -> None:
    ...
    from anima.config import settings
    db.init(settings.db_path)
    from anima.dataset.manager import build
...
def classifier_train() -> None:
    ...
    from anima.inference.classifier import train
    clf = train(save=True)
...
def classifier_benchmark(llm_sample: int) -> None:
    ...
    import anima.inference.intent_engine as engine
    ...
    from anima.inference.benchmark import run_full_benchmark
...
def frontdoor_enter() -> None:
    ...
    from anima.kernel.frontdoor import (
        EntryActionKind,
        FrontDoorPolicy,
        decide_entry,
    )

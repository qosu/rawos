...
log = logging.getLogger("anima.api.timing")
...
def timing_signals(user: User = Depends(current_user)) -> dict:
    ...
    from anima.timing.signals import compute_signals
...
def timing_score(user: User = Depends(current_user)) -> dict:
    ...
    from anima.timing.model import get_timeliness, TIMELINESS_THRESHOLD
    from anima.context.user_model import get_user_model
